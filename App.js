...
const RestaurantCard = ({ recipe }) => {
  const { name, image, rating, cuisine, prepTimeMinutes, ingredients } = recipe
  return (
    <div className='res-card'>
      ...
      <h4>{rating}</h4>
      <h4>{cuisine}</h4>
      <h4>{prepTimeMinutes}</h4>
      <h4>Ingredients: <span className='normal-text'>{ingredients.join(", ")}</span></h4>
  ...
  return (
    <div className='body'>
      ...
      <div className='Resto-container'>
        {resObj.recipes.map(res => (
          <RestaurantCard key={res.id} recipe={res} />
        ))}
      </div>
    </div>
  )
